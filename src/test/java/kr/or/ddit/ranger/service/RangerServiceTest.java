...
public class RangerServiceTest {
	...
	//하나의 메서드도 시나리오에 따라서는 여러개의 테스트 메소드를 생성 할 수도 있다.
	//ex) 회원등록 dao메소드 생성시 정상적인 값을 입력해서 db 테이블에 데이터가 정상적으로 입력되는 성공케이스
	//    비정상적인 값(회원 id중복)을 입력하여 db테이블에 데이터가 입력되지 않는 실패 케이스
	
	
	// 테스트 메소드는 테스트하려고하는 메소드이름 + 접미어Test
	// getRangersTest
	...
	// main 메소드가 없음에도 실행이 가능한 것은 이클립스에서@test 메소드가 붙은 메소드를 실행 해주기 때문 (junit에서 실행)
	// maven 배포시에도 @test 메소드가 붙은 메소드가 통과가 되어야 빌드 사이클이 진행 
